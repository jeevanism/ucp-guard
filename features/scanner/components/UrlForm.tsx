...
import { ShieldAlert, Terminal, AlertTriangle, Settings2, Check } from "lucide-react";
...
const AVAILABLE_MODELS = [
  { id: "gemini-2.0-flash", name: "Gemini 2.0 Flash (Recommended)", description: "High speed, high rate limits." },
  { id: "gemini-2.0-flash-lite-preview-02-05", name: "Gemini 2.0 Flash Lite", description: "Fastest, lightweight model." },
  { id: "gemini-1.5-flash", name: "Gemini 1.5 Flash", description: "Legacy stable model." },
  { id: "gemini-1.5-pro", name: "Gemini 1.5 Pro", description: "Higher reasoning, lower rate limits." },
];
...
export function UrlForm({ onScanStart, isLoading, serverError }: UrlFormProps) {
  ...
  const isRateLimitError = serverError?.includes("RATE LIMIT") || serverError?.includes("429");
          ...
          <div className="space-y-2">
            ...
            {(showSettings || isRateLimitError) && (
              <div className={cn(
                ...
                isRateLimitError ? "bg-amber-950/20 border-amber-900/50" : "bg-zinc-950/50 border-zinc-800"
              )}>
                 ...
                 <div className="grid gap-2">
                    {AVAILABLE_MODELS.map((m) => (
                      ...
                      >
                        ...
                        {modelId === m.id && <Check className="h-3 w-3 text-indigo-400" />}
                      </div>
                    ))}
                 </div>
                 {isRateLimitError && (
                   <p className="text-xs text-amber-500 mt-2 font-mono">
                     * Recommendation: Switch to "Flash Lite" if you are hitting quotas.
                   </p>
                 )}
              </div>
            )}
            ...
            {serverError && (
              <div className={cn(
                ...
                isRateLimitError ? "text-amber-400 bg-amber-950/20 border border-amber-900/50" : "text-red-400 bg-red-950/20 border border-red-900/50"
              )}>
                <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" />
                <div className="space-y-1">
                    <p className="font-bold">SYSTEM ERROR</p>
                    <p>{serverError}</p>
                    {isRateLimitError && <p className="font-bold underline cursor-pointer" onClick={() => setShowSettings(true)}>Change Model above to fix this.</p>}
                </div>
              </div>
            )}
          </div>
